...
export default {
  Query: {
    notificaciones: () => Notificacion.findAll({
      include: [
        {
          model: Usuario,
          as: 'usuario',
    ...
    totalNotificaciones: () => Notificacion.findAndCountAll().then((result) => result.count),
  ...
  Mutation: {
    ...
    deleteNotificacion: (parent, args) => Notificacion.destroy({ where: { id: args.id } }),
  },
};
